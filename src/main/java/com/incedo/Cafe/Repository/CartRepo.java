...
public class CartRepo  {
    ...
    public int insert_details(Cart cart) {
        ...
        int Id_test =(int) jdbcTemplate.queryForObject("select cart_id from cafe_cart where id = (select max(id) from cart)",Integer.class);
    ...
    public void updateStatus(Cart carts) {
        jdbcTemplate.update("update cafe_cart set payment_status='"+carts.getPayment_status()+"',txn_id = '"+carts.getTnx_id()+"' where cart_id="+carts.getCart_id()+";");
    }
}
